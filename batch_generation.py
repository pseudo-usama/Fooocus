...
def generate_in_batch(batches, parent_dir="batch_generation", test=False):
    for i, batch in enumerate(batches):
        quality = 'Speed' if test else 'Quality'
        size = batch["aspect_ratio"]
        n = 5 if test else 10
        dir_path = f"{parent_dir}/{i}. {batch['theme']}"
